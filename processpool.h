...
/*进程池类，将他定义为模板类是为了代码复用。其参数是处理逻辑任务的类
T指代每个子进程对应的user类型*/
template< typename T >
class processpool
{
...
/*从epollfd标识的的epoll内核事件表中删除fd上的所有注册事件*/
...
/*进程池构造参数。参数listenfd是监听socket，它必须在创建进程池之前被创建，
...
void processpool< T >::setup_sig_pipe()
{
    ...
    /*设置信号处理函数，信号处理函数向sig_pipefd[1]写信号值*/
...
void processpool< T >::run_child()
{
    ...
    /*子进程需要监听管道文件描述符pipefd，因为父进程将通过它来通知子进程accpet新连接*/
    ...
    users = NULL;
    close( pipefd );
    /*注释的原因：因该由m_listenfd的创建者来关闭这个文件描述符(见后)，
                        ...
                        switch( signals[i] )
                        {
                            case SIGCHLD:
                            {
                                ...
                                while ( ( pid = waitpid( -1, &stat, WNOHANG ) ) > 0 )
                                {
                                    ...
                                    {
/*如果进程池中第i个子进程退出了，则主进程关闭 相应的通信管道，并设置相应的m_pid为-1，
以标记该子进程已经退出*/
